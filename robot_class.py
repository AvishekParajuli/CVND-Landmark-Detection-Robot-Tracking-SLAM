...
class robot:
    ...
    def sense(self):
        ...
        for index in range(self.num_landmarks):
            ...
            #change it to match format: L(k) = X0 +dist => dist = L(k)- X(0)
            dist_x = curr_landmark[0] - self.x + self.rand() * self.measurement_noise
            dist_y = curr_landmark[1] - self.y + self.rand() * self.measurement_noise
